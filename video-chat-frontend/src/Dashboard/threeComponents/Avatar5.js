...
export default function Model(_state, { ...props }) {
  const group = useRef();
  ...
  useFrame((state, delta) => {
    ...
    if (!wPress) {
      ...
      if (animation != "idle") {
        setAnimation("idle");
        _state.store.users[`${_state.userID}`].animation = "idle";
      }
    }
    ...
    if (wPress) {
      ...
      _state.store.users[`${_state.userID}`].position = storeUpdate;
      if (animation != "walk") {
        setAnimation("walk");
        _state.store.users[`${_state.userID}`].animation = "walk";
      }
    }
    ...
    if (aPress) {
      ...
      _state.store.users[`${_state.userID}`].rotation = storeUpdate;
    }
    if (dPress) {
      group.current.rotation.y = group.current.rotation.y - rotateAngle;
      let storeUpdate = {
  ...
  return (
    <group ref={group} {...props} dispose={null}>
      <WebCamScreen videoElement={_state.videoElement} />
